...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fetch non-sensitive settings (ai_api_key is NOT in this table)
    ...
    // AI API key lives in Supabase Edge Function Secrets — never in the DB
    ...
    for (const task of tasks) {
      ...
      await fetch(`${adminUrl}notifications`, {
        ...
        body: JSON.stringify({
          user_id: task.assigned_to,
          type: "ai_fallback",
          message: `AI generated your ${task.content_type} on "${task.topic}" — please upload to Instagram.`,
        }),
      });
...
async function generateContent(
  ...
  if (!provider || !apiKey) {
    return (
      ...
      `Set AI_API_KEY in Supabase Edge Function Secrets and choose a provider in Settings.`
    );
  }
